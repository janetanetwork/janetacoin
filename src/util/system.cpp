...
const char * const BITCOIN_CONF_FILENAME = "janetacoin.conf";
...
#else
    const char* pszModule = "janetacoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Janetacoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Janetacoin";
#else
    // Unix-like
    return pathRet / ".janetacoin";
#endif
#endif
}
