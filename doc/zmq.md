...
The Janetacoin Core daemon can be configured to act as a trusted "border
router", implementing the janetacoin wire protocol and relay, making
...
The ZeroMQ feature in Janetacoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building janetacoind:
...
For instance:
    ...
    $ janetacoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/janetacoind.tx.raw \
...
These options can also be provided in janetacoin.conf.
...
From the perspective of janetacoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into janetacoind directly. Furthermore, no information is
...
using. Janetacoind appends an up-counting sequence number to each
